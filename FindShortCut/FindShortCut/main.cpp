...
using namespace std;
//const int MAX_QUANTITY_VERTEX = 50000;
...
int GetQuantityTriangles(string FileName)
{
	FILE *file_stl;
	uint32_t temp_32;
...
bool InitializationFileStl(string FileName, int *Quantity_vertex, Vertex* Massive_Vertex, int MAX_QUANTITY_VERTEX)         //File initialization
{
	FILE *file_stl;
	uint8_t *buffer = new uint8_t[80];
	uint32_t temp_32;
	...
	if (file_stl = fopen(FileName.c_str(), "rb"))
	{
		...
		cout << temp_32 << endl;
		//Massive_Vertex = new Vertex[temp_32];
...
float bfs(int start, int end, int* Quantity_vertex, int *length_road, int *Index_way, Vertex * Massive_Vertex, int MAX_QUANTITY_VERTEX) //Bfs
{

	*length_road = 0;
	bool *VisitedPoint= new bool[MAX_QUANTITY_VERTEX];            // for Djekstru
	float *DistantPoint= new float[MAX_QUANTITY_VERTEX];       // for Djekstru
	int index_min = 0;
	queue <int> turn;
	...
	while (!turn.empty())       //while the queue not emptys
	{
		...
		for (int i = 0; i<Massive_Vertex[ind].Get_count_related_vertex(); i++)      //find the next Vertex
		{
			...
			{
				VisitedPoint[Massive_Vertex[ind].Get_Related_Vertex(i)] = false;
...
int main(int argc, char* argv[])
{
	...
	Vertex *Massive_Vertex = new Vertex[GetQuantityTriangles(FileName)];
	int MAX_QUANTITY_VERTEX = GetQuantityTriangles(FileName) - 1;
	int *pIndex_way= new int[MAX_QUANTITY_VERTEX];
	int *Index_way = pIndex_way;
	if (!InitializationFileStl(FileName, Quantity_vertex, Massive_Vertex, MAX_QUANTITY_VERTEX))
		return -1;
	do
	{
		...
		if (ifstart && ifend)
		{
			float way = bfs(start, end, Quantity_vertex, length_road, Index_way, Massive_Vertex, MAX_QUANTITY_VERTEX);
			if (way <= 0)
